...
Este trabajo se centra en el procesamiento de datos en el ámbito del análisis de la calidad del aire, abordando un conjunto de datos recopilado durante los años 2019-2022. Inicialmente, el dataset presentaba desafíos inherentes, como la presencia de valores faltantes y una estructura no tidy. La tarea principal consistió en transformar esta información en un formato "tidy", eliminamos valores ausentes y garantizamos la coherencia de los datos.
...
En un primer paso, se emprendió la tarea de obtener un conjunto de datos consolidado a partir de nuestros registros. Este proceso involucró un análisis detallado de cada año, con el objetivo de integrar las columnas correspondientes a las observaciones diarias de diversos agentes químicos del aire presentes en cada hoja de Excel. La finalidad era fusionar estas columnas en una única entidad independiente, que contuviera todas las observaciones de dicho parámetro durante los cuatro años contemplados en el estudio.

Este enfoque permitió la generación de un conjunto de datos estructurado de manera "tidy", donde cada variable se representaba en una columna individual y cada observación de una variable ocupaba una fila independiente. La consolidación de las diversas tablas y hojas de Excel se llevó a cabo considerando las variables comunes, es decir, los parámetros del aire analizados. Como resultado, nuestro nuevo dataset presentaba una primera fila que contenía los nombres representativos de las variables, estableciendo así un marco coherente para el análisis subsiguiente.
